<template>
  ...
  >
    ...
    <div v-if="isLoading" class="loading-state topography-overlay">Loading heatmap data...</div>
    ...
    <div v-if="showCanvas" class="topography-hint">Drag to pan · Scroll to zoom</div>
  </div>
</template>
...
const { data, error, heatmapMaxMm, heatmapMinMm, isLoading, meta, pixelToMmFactor } =
  useAnalysisHeatmap({
...
const axes = computed(() => {
  if (!surface.value || !meta.value) {
    return null
  }

  return buildHeatmapTopographyAxes({
    fps: meta.value.fps,
    heatmapMaxMm: heatmapMaxMm.value,
    heatmapMinMm: heatmapMinMm.value,
    metrics: surface.value.metrics,
...
const clipPlaneHeight = computed(() => {
  if (
    !surface.value ||
    props.clipPlaneMm === null ||
    props.clipPlaneMm === undefined ||
    !Number.isFinite(props.clipPlaneMm)
  ) {
    return null
  }

  return mapHeatmapMmValueToTopographyHeight(
    props.clipPlaneMm,
    heatmapMinMm.value,
    heatmapMaxMm.value,
    surface.value.metrics.heightScale,
  )
})
...
watch(
  [surface, frameLine, axes, hasSurface, clipPlaneHeight, () => props.clippingEnabled ?? false],
  () => {
    applySceneSurface()
    applySceneFrameLine()
    applySceneAxes()
...
function applySceneAxes(): void {
  if (!sceneController) {
    return
  }

  sceneController.setAxes(hasSurface.value ? axes.value : null)
}

function applySceneClipping(): void {
  if (!sceneController) {
    return
  }

  if (!hasSurface.value || !props.clippingEnabled) {
    sceneController.setClippingHeight(null)
    return
  }

  sceneController.setClippingHeight(clipPlaneHeight.value)
}
