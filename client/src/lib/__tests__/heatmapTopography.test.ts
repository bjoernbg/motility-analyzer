...
describe('heatmap topography utilities', () => {
  ...
  it('downsamples wide analyses to a bounded number of topography columns', () => {
  ...
  it('maps world coordinates back to the original frame and point indices', () => {
    ...
    const line = buildHeatmapTopographyFrameLine({
      ...
      pixelToMmFactor: 10,
      values: new Float32Array(30).fill(30),
    });

    expect(line).not.toBeNull();
    expect(line?.positions[0]).toBeCloseTo(1.5);
    expect(line?.positions[2]).toBeGreaterThan(surface.metrics.heightScale * 0.5);
  });
});
