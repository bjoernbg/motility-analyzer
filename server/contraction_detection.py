...
def detect_contractions(
    ...
    Y, T = thickness.shape
    logger.info(
        f"Starting contraction detection: input shape ({Y}, {T}) = {Y} points × {T} frames"
    )
    ...
    logger.info(
        f"Preprocessing complete: smoothed with sigma=({smooth_sigma[0]}, {smooth_sigma[1]})"
    )

    # Threshold on normalized thickness
    logger.info(
        f"Thresholding: using {'manual' if thr is not None else f'{percentile}th percentile'} threshold..."
    )
    ...
    logger.info(
        f"Thresholding complete: threshold={used_thr:.4f}, {mask_pixels} pixels ({mask_percent:.1f}%) below threshold"
    )

    # Clean and label
    logger.info(
        f"Cleaning and labeling: opening iterations={open_iters}, closing iterations={close_iters}..."
    )
    mask_c, lbl, n = clean_and_label(
        mask, open_iters=open_iters, close_iters=close_iters
    )
    ...
    logger.info(
        f"Contraction detection complete: detected {len(events)} contraction event(s)"
    )
