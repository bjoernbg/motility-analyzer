    ...
    try:
        with open(settings_path, 'r') as f:
            ...
            if 'metadata' in settings_data:
                ...
                required_fields = ['total_frames', 'fps', 'width', 'height', 'duration', 'file_size']
...
def generate_metadata_from_video(video_path: Path) -> VideoMetadata:
    ...
    try:
        ...
        ffprobe = FFmpeg(executable="ffprobe").input(video_path, print_format="json", show_streams=None)
        ...
        for i in range(500):  # Check first ~500 frames
            ret, frame = cap.read()
            if not ret:
                break
        ...
        actual_frame = int(container_frame_pos / frame_multiplier) if frame_multiplier > 0 else int(container_frame_pos)
...
def get_video_metadata(video_path: Path, force_regenerate: bool = False) -> VideoMetadata:
