...
def intersect_normal_with_path(
    ...
    target_segments: tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]
    | None = None,
) -> tuple[float, float] | None:
...
def project_perpendicular(
    ...
    intersection = intersect_normal_with_path(
        center_point, normal, target_path_np, target_segments
    )
...
def calculate_center_path(
    ...
    smoothed_path = [
        ...
        for (x, _), y_smooth in zip(center_path, smoothed_y, strict=False)
    ]
...
def calculate_path_arc_length(
    path: list[tuple[int, int]], start_idx: int = 0, end_idx: int | None = None
) -> float:
    ...
    if distribution_method == "center_line_projection":
        ...
        if total_length < 1e-6:
            ...
            if num_tracking_points > 1:
                target_lengths = [
                    i * total_length / (num_tracking_points - 1)
            ...
            for target_length in target_lengths:
                ...
                while path_idx < len(center_in_window) - 1:
                    ...
                    if (
                        accumulated_length + segment_length >= target_length
                        or path_idx == len(center_in_window) - 2
                    ):
        ...
        left_dist2 = (left_pts[:, 0] - center_points_np[:, 0]) ** 2 + (
            left_pts[:, 1] - center_points_np[:, 1]
        ) ** 2
        right_dist2 = (right_pts[:, 0] - center_points_np[:, 0]) ** 2 + (
            right_pts[:, 1] - center_points_np[:, 1]
        ) ** 2
        ...
        for center_point, center_idx_raw in zip(
            center_in_window, center_indices, strict=False
        ):
...
async def process_video(
    ...
    if start_frame > 0 and existing_results:
        ...
        if previous_frames:
            ...
            prev_path_top = (
                [tuple(pt) for pt in last_frame.pt] if last_frame.pt else None
            )
            prev_path_bottom = (
                [tuple(pb) for pb in last_frame.pb] if last_frame.pb else None
            )
    ...
    try:
        ...
        for frame_num in range(start_frame, total_frames):
            ...
            if frame_num % chunk_size == 0:
                ...
                logger.warning(
                    f"progress: {progress:.2f}% (frame {frame_num + 1} of {total_frames}) @ {(chunk_size / elapsed_time):.2f}fps"
                )
                time_start = time.perf_counter()
    ...
    global_data = {
        ...
        "average_points_top_per_frame": total_points_top / total_frames
        if total_frames > 0
        else 0,
        "average_points_bottom_per_frame": total_points_bottom / total_frames
        if total_frames > 0
        else 0,
        "average_points_center_per_frame": total_points_center / total_frames
        if total_frames > 0
        else 0,
        "average_regions_per_frame": total_regions / total_frames
        if total_frames > 0
        else 0,
        "parameters_used": {
            "smoothing_factor": parameters.smoothing_factor,
